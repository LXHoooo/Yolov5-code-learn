...
import argparse  # python的命令行解析的标准模块，可以让我们直接在命令行中就可以向程序传入参数并运行程序
import os
import sys  # sys系统模块，包含了与python解释器和它的环境有关的函数
from pathlib import Path  # pah将str转换为path对象，使字符串路径易于操作的模块

import torch
import torch.backends.cudnn as cudnn  # cuda模块

FILE = Path(__file__).resolve()  # 将文件路径等加入系统的环境变量，该脚本结束后失效
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # 将预测信息（相对img_size 640）映射回原图img0 size
...
def parse_opt():
    ...
    save-txt:是否将预测的框坐标以txt文件格式保存，默认True
    save-conf:是否保存预测每个目标的置信度到预测txt文件中
    save-crop:是否需要将预测到的目标从原图中抠出来，剪切好并保存
    nosave:是否不要保存预测后的图片
    classes:在nms中是否只保留某些特定的类
    agnostic-nms:进行nms是否也出去不同类别之间的框
    argument:预测是否也要采用数据增强
